    ...
    opts.files.forEach((file) => {
        ...
        if (LOGS) {
            ...
            db.on('insert', (item) => {
                ...
                // console.log(i.sid);
                if (opts.game) {
                    if (!i.sid || !~i.sid.indexOf(opts.game)) return false;
                }
...
const setDefaults = (LOGS, serverConf, opts, game) => {
    ...
    opts.header = opts.csvHeader ?? 'all';
    opts.flatten = !!opts.csvFlatten;
    if ('string' === typeof opts.csvFlatten) {
        opts.flattenByGroup = opts.csvFlatten;
    }
